...
export default function HomePage() {
  ...
  return (
    <motion.div variants={staggerContainer} initial="hidden" animate="show" className="flex flex-col gap-8">
      ...
      <motion.div variants={fadeUp} className="hero-console relative overflow-hidden rounded-xl bg-navy text-white p-5 sm:p-7 lg:p-9 shadow-lg shadow-navy/10">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1fr)_240px] lg:items-center">
          <div className="max-w-4xl"><span className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-[10px] font-mono font-semibold uppercase tracking-widest text-saffron"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse" /> GIGW 3.0 Autonomous Compliance</span><h2 className="mt-4 text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-balance">TenderVerify AI <span className="text-saffron">—</span> Procurement Intelligence &amp; Compliance Console</h2><p className="mt-4 max-w-3xl text-sm leading-6 text-navy-100">A single console reconciling every bidder declaration against live GSTN, MCA-21, Udyam, EPFO/ESIC, GeM and allied statutory registries — turning a manual, document-by-document eligibility check into a deterministic, auditable clearance in minutes.</p><div className="mt-6 flex flex-col xs:flex-row flex-wrap gap-3"><Link to={isBidder ? '/documents' : isOfficer ? '/evaluation-matrix' : '/login'} className="flex items-center justify-center gap-1.5 px-4 py-2.5 bg-saffron hover:bg-orange-300 text-navy rounded-md font-bold text-[12px] shadow-sm"><Icon name={isBidder ? 'folder_open' : 'fact_check'} size={16} /> {isBidder ? 'Submit Your Bid Documents' : 'Open NIT-882 Evaluation Matrix'} <Icon name="arrow_forward" size={15} /></Link><Link to="/tender-notices" className="flex items-center justify-center gap-1.5 px-4 py-2.5 border border-white/25 hover:bg-white/10 text-white rounded-md font-semibold text-[12px]">View tender notices</Link></div></div>
          <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: .35, duration: .6 }} className="hidden sm:block rounded-xl border border-white/15 bg-white/10 p-4 backdrop-blur-sm">
            ...
            <div className="mt-5 flex items-center justify-center"><motion.div animate={{ rotate: 360 }} transition={{ duration: 18, repeat: Infinity, ease: 'linear' }} className="relative flex h-28 w-28 items-center justify-center rounded-full border border-dashed border-saffron/50"><div className="absolute inset-2 rounded-full border border-white/10" /><Icon name="verified" size={42} className="text-saffron" /></motion.div></div>
